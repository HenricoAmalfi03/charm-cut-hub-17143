...
import { ArrowLeft, DollarSign, Scissors, TrendingUp, Users, FileDown } from 'lucide-react';
...
export default function AdminRelatorios() {
  ...
  const gerarRelatorioPDF = async () => {
    try {
      ...
      const detalhes = await Promise.all(
        (agendamentosCompletos || []).map(async (ag: any) => {
          const [cliente, barbeiro, servico] = await Promise.all([
            supabase.from('clientes').select('nome_completo').eq('id', ag.cliente_id).single(),
            supabase.from('barbeiros').select('nome_completo').eq('id', ag.barbeiro_id).single(),
            supabase.from('servicos').select('nome, preco').eq('id', ag.servico_id).single()
          ]);
          return {
      ...
      doc.setFontSize(10);
      doc.text(`Total de Agendamentos: ${relatorio.totalAgendamentos}`, 14, yPos);
      yPos += 6;
      doc.text(`Agendamentos Concluídos: ${relatorio.agendamentosConcluidos}`, 14, yPos);
      yPos += 6;
      doc.text(`Receita Total: R$ ${relatorio.receitaTotal.toFixed(2)}`, 14, yPos);
      yPos += 6;
      ...
      Object.keys(porBarbeiro).forEach((nomeBarbeiro) => {
        ...
        yPos += 6;
        doc.setFontSize(10);
        doc.text(`Atendimentos: ${atendimentos.length} | Clientes: ${totalClientes} | Receita: R$ ${totalReceita.toFixed(2)}`, 14, yPos);
        yPos += 8;
      ...
      doc.save(`relatorio-barbearia-${new Date().toISOString().split('T')[0]}.pdf`);
